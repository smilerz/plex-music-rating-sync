...
class TrackPair(SyncPair):
    ...
    def match(self, candidates=None, match_threshold=30):
        ...
        if score < match_threshold:
            self.sync_state = SyncState.ERROR
            self.logger.debug("Score of best candidate {} is too low: {} < {}".format(self.destination_player.format(candidates[ranks[-1]]), score, match_threshold))
            return score

        self.destination = candidates[ranks[-1]]
        self.logger.debug("Found match with score {} for {}: {}".format(score, self.source, self.destination_player.format(self.destination)))
        if score != 100:
            self.logger.info("Found match with score {} for {}: {}".format(score, self.source, self.destination_player.format(self.destination)))
        ...
        if self.rating_source == self.rating_destination:
            ...
            self.logger.warning("Found match with conflicting ratings: {} (Source: {} | Destination: {})".format(self.source, self.rating_source, self.rating_destination))
        ...
        if self.destination is None:
            ...
            for pair in track_pairs:
                if not self.destination.has_track(pair.destination):
                    self.destination_player.update_playlist(self.destination, pair.destination, True)
