...
class Rating:
    def __init__(self, raw: Union[float, str], scale: Optional[RatingScale] = None, *, aggressive: bool = False) -> None:
        ...
        self.normalized = self._normalize(value, self.scale)

    @staticmethod
    def infer(value: float, aggressive: bool = False) -> Optional[RatingScale]:
        popm_values = {byte for _, byte in RATING_MAP[1:]}  # exclude zero
    ...
    def to_float(self, scale: Optional[RatingScale] = None) -> float:
        ...
        if target_scale == RatingScale.POPM:
            return self._to_popm(self.normalized)
        return round(self.normalized * target_scale.value, 3)
    ...
    def to_display(self, scale: RatingScale = RatingScale.ZERO_TO_FIVE) -> str:
        ...
        return f"{round(value, 1)} / {scale.value:.1f}"
    ...
    def _from_popm(self, byte: float) -> float:
        if byte <= 0:
            return 0.0
        best_match = min(RATING_MAP[1:], key=lambda pair: abs(pair[1] - byte))
        return best_match[0]

    def _to_popm(self, normalized: float) -> int:
        for rating, byte in RATING_MAP[1:]:
    ...
    def __repr__(self) -> str:
        return f"<Rating normalized={self.normalized:.3f}>"
