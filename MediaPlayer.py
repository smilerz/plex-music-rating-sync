...
class MediaPlayer(abc.ABC):
    ...
    def get_normed_rating(self, rating: Optional[float]):
        return max(0, rating) / self.rating_maximum
...
class MediaMonkey(MediaPlayer):
    ...
    def create_playlist(self, title: str, tracks: List[object]):
        ...
        if not self.dry_run and tracks:
            playlist = self.sdb.PlaylistByTitle("").CreatePlaylist(title)
            for track in tracks:
                song = self.sdb.Database.QuerySongs("ID=" + str(track.ID))
                playlist.AddTrack(song.Item)

    def find_playlist(self, **kwargs):
    ...
    def read_child_playlists(self, parent_playlist):
        ...
        for i in range(len(parent_playlist.ChildPlaylists)):
            ...
            if playlist.is_auto_playlist:
                self.logger.debug(
                    "Skipping to read tracks for auto playlist {}".format(playlist.name)
                )
                continue
    ...
    def update_playlist(self, playlist, track, present):
        self.logger.debug(
            "{} {} to playlist {}".format(
                "Adding" if present else "Removing", self.format(track), playlist.name
            )
        )
        if not self.dry_run:
    ...
    def update_rating(self, track, rating):
        self.logger.debug(
            'Updating rating of track "{}" to {} stars'.format(
...
class PlexPlayer(MediaPlayer):
    ...
    def connect(self, server, username, password="", token=""):
        self.logger.info(
            f"Connecting to the Plex Server {server} with username {username}."
        )
        connection_attempts_left = self.maximum_connection_attempts
        while connection_attempts_left > 0:
            time.sleep(
                1
            )  # important. Otherwise, the above print statement can be flushed after
            ...
            try:
                ...
                print(
                    f"Username {username}, password or token wrong for server {server}."
                )
                password = ""
        ...
        if connection_attempts_left == 0 or self.account is None:
            self.logger.error(
                "Exiting after {} failed attempts ...".format(
        ...
        self.logger.info(
            "Connecting to remote player {} on the server {}".format(
                self.name(), server
            )
        )
        try:
        ...
        music_libraries = {
            section.key: section
            for section in self.plex_api_connection.library.sections()
    ...
    def create_playlist(
        self, title, tracks: List[plexapi.audio.Track]
    ) -> Optional[plexapi.playlist.Playlist]:
        ...
        if self.dry_run:
            ...
            if tracks is None or len(tracks) == 0:
                self.logger.warning(
                    "Playlist {} can not be created without supplying at least one track. Skipping.".format(
                        title
                    )
                )
                return None
    ...
    def find_playlist(self, **kwargs) -> Optional[plexapi.playlist.Playlist]:
        ...
        try:
            ...
            self.logger.debug(
                "Playlist {} not found on the remote player".format(title)
            )
            return None
    ...
    def search_tracks(self, key: str, value: Union[bool, str]) -> List[AudioTag]:
        ...
        if key == "title":
            ...
            for x in matches:
                ...
                counter += 1
            self.logger.info(
                "Found {} tracks with a rating > 0 that need syncing".format(counter)
            )
            matches = tags
        else:
            raise KeyError(f"Invalid search mode {key}.")
    ...
    def update_playlist(self, playlist, track, present):
        ...
        if present:
            self.logger.debug(
                "Adding {} to playlist {}".format(self.format(track), playlist.title)
            )
            if not self.dry_run:
                playlist.addItems(track)
        else:
            self.logger.debug(
                "Removing {} from playlist {}".format(
                    self.format(track), playlist.title
                )
            )
            if not self.dry_run:
    ...
    def update_rating(self, track, rating):
        ...
        if not self.dry_run:
            try:
                ...
                song = [
                    s
                    for s in self.music_library.searchTracks(title=track.title)
                    if s.key == track.ID
                ][0]
